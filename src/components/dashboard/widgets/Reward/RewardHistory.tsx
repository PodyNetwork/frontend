            ...
            : claimHistory.length > 0 &&
              claimHistory.map((data, index) => (
                <motion.li
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 0.3 * 0.1, duration: 0.5 }}
                  className="flex flex-row flex-wrap items-center justify-between bg-white rounded-xl px-3 sm:px-6 py-3 mb-2 __shadow_pody transition-shadow duration-300"
                >
                  <motion.div
                    className="flex items-center gap-x-2 flex-grow min-w-0"
                    initial={{ opacity: 0 }}
