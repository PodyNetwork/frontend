...
import { formatNum } from "@/func/formatNum";
...
export const LeaderboardItem = ({
  ...
  >
    <div className="flex items-center gap-x-2 flex-grow min-w-0">
      <div className="text-base text-slate-500 w-8 flex-shrink-0 font-semibold">
        {rank}
      </div>
      <div className="flex items-center gap-x-2 sm:gap-x-3 min-w-0">
        ...
        <h3 className="text-xs sm:text-sm text-slate-500 truncate">
          {username}
        </h3>
      </div>
    </div>
    <div className="flex-shrink-0 ml-2">
      <p className="text-xs sm:text-sm flex flex-row items-center gap-x-1 font-bold text-pody-primary whitespace-nowrap">
        {formatNum(Number(formatUnits(BigInt(totalPoints), 18)))}
      </p>
    </div>
  </motion.li>
);
