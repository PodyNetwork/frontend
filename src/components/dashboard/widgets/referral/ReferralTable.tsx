...
const ReferralTable = () => {
  ...
  const {
    ...
    isLoading,
    totalPages,
  } = useGetReferrals({ page, limit, sortDirection, dateJoined });
      ...
      <table className="hidden md:table w-full text-sm text-left rtl:text-right text-slate-500">
        ...
        <tbody className="__pdy_tbl_bdy">
          {referralData.map((data: ReferralData, index: number) => (
            <tr
              key={data._id} // Use _id as the key for better stability
              className="bg-white border-b border-slate-100 text-sm hover:bg-slate-50"
            >
              <td className="w-4 p-4">{(page - 1) * limit + index + 1}</td>
              <td
                scope="row"
  ...
  <div className="w-full p-8 mx-auto max-w-sm">
    ...
    <div className="text-center">
      <h3 className="text-lg font-medium text-slate-800">
        You don&apos;t have a Referral Currently
      </h3>
      <p className="text-xs mt-2 text-slate-700">
        Take advantage of an exciting opportunity to earn rewards! Copy your
        unique referral link and share it with your friends and network.
      </p>
    </div>
  </div>
);
